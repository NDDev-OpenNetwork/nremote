...
#define NREMOTE_LIB_PATH "libnremote.so"
typedef bool (*NRemoteCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

// The bundle keeps the core library at lib/libnremote.so next to the
...
static void* dlopen_bundled_lib() {
  ...
  *last_slash = '\0';
  char lib_path[PATH_MAX + sizeof("/lib/" NREMOTE_LIB_PATH)];
  snprintf(lib_path, sizeof(lib_path), "%s/lib/%s", exe_path, NREMOTE_LIB_PATH);
  if (access(lib_path, F_OK) != 0) return nullptr;
  void* libnremote = dlopen(lib_path, RTLD_LAZY);
  if (!libnremote) {
    char* error = dlerror();
    if (error != nullptr) {
...
bool flutter_nremote_core_main() {
   void* libnremote = dlopen_bundled_lib();
   if (!libnremote) {
      libnremote = dlopen(NREMOTE_LIB_PATH, RTLD_LAZY);
   }
   if (!libnremote) {
      fprintf(stderr,"Failed to load \"libnremote.so\"\n");
      char* error;
   ...
   auto core_main = (NRemoteCoreMain) dlsym(libnremote,"nremote_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"nremote_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
