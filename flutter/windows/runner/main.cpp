...
typedef char** (*FUNC_NREMOTE_CORE_MAIN)(int*);
typedef void (*FUNC_NREMOTE_FREE_ARGS)( char**, int);
typedef int (*FUNC_NREMOTE_GET_APP_NAME)(wchar_t*, int);
typedef int (*FUNC_NREMOTE_IS_DISABLE_INSTALLATION)();
...
{
  HINSTANCE hInstance = LoadLibraryA("libnremote.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libnremote.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_NREMOTE_CORE_MAIN nremote_core_main =
      (FUNC_NREMOTE_CORE_MAIN)GetProcAddress(hInstance, "nremote_core_main_args");
  if (!nremote_core_main)
  {
    std::cout << "Failed to get nremote_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_NREMOTE_FREE_ARGS free_c_args =
      (FUNC_NREMOTE_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = nremote_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "NRemote [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);
  FUNC_NREMOTE_IS_DISABLE_INSTALLATION nremote_is_disable_installation =
      (FUNC_NREMOTE_IS_DISABLE_INSTALLATION)GetProcAddress(hInstance, "nremote_is_disable_installation");
  bool is_disable_installation =
      nremote_is_disable_installation && nremote_is_disable_installation() != 0;
  ...
  std::wstring app_name = L"NRemote";
  FUNC_NREMOTE_GET_APP_NAME get_nremote_app_name = (FUNC_NREMOTE_GET_APP_NAME)GetProcAddress(hInstance, "get_nremote_app_name");
  if (get_nremote_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_nremote_app_name(app_name_buffer, 512) == 0) {
