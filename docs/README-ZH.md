...
NRemote 期待各位的贡献. 如何参与开发? 详情请看 [CONTRIBUTING-ZH.md](CONTRIBUTING-ZH.md).
...
cd nremote # 进入文件夹
docker build -t "nremote-builder" . # 构建容器
```
...
* 针对国内网络访问问题，可以做以下几点优化：  
   ...
   4. docker build 命令后面加上 proxy 参数
      ...
      ```
      docker build -t "nremote-builder" . --build-arg http_proxy=http://host:port --build-arg https_proxy=http://host:port
      ```
...
生成的可执行程序在 target 目录下，可直接通过指令运行调试 (Debug) 版本的 NRemote:
```sh
...
请注意：
* 请保证您运行的目录是在 NRemote 库的根目录内，否则软件会读不到文件。
...
- **[src/rendezvous_mediator.rs](https://github.com/rustdesk/rustdesk/tree/master/src/rendezvous_mediator.rs)**: 与[nremote-server](https://github.com/rustdesk/rustdesk-server)保持UDP通讯, 等待远程连接（通过打洞直连或者中继）
