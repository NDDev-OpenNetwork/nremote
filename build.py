...
hbb_name = 'nremote' + ('.exe' if windows else '')
...
def get_features(args):
    ...
    if args.drm:
        ...
        # package NAMED `nremote` carrying the consent-bypass backend and the root-side uinput
...
# libdrmtap is fetched at build time from the nremote-org fork at a pinned
# commit — the same way nremote sources its other native build deps (vcpkg,
# flutter_rust_bridge, ...), rather than carrying a git submodule. It is the ONLY
# pin for the drm backend: nremote dlopens this .so at runtime and does not depend on
...
def build_libdrmtap_so():
    # Build libdrmtap.so from the nremote-org fork, fetched at the pinned LIBDRMTAP_SHA. The
    ...
    # the shared one explicitly (nremote dlopens the .so and never needs the archive).
...
DRM_PACKAGE_NAME = 'nremote-unattended-wayland'
...
def stage_libdrmtap_into_deb(so_path):
    ...
    # symlink is needed: libdrmtap is resolved by ABSOLUTE path (/usr/lib/nremote/libdrmtap.so.0) at
    # the in-process dlopen site (drmtap_dl.rs), so the deb does NOT drop /usr/lib/nremote into the
    ...
    system2('mkdir -p tmpdeb/usr/lib/nremote')
    ...
    system2(f'cp "{so_path}" tmpdeb/usr/lib/nremote/')
    system2(f'ln -sf "{so_basename}" tmpdeb/usr/lib/nremote/libdrmtap.so.0')


def _max_glibc_minor(path):
    # Read from .dynstr rather than via objdump so packaging needs no binutils; chunked because
    # libnremote.so is ~45 MB.
    best = 0
...
def measured_glibc_floor():
    ...
    paths = [p for p in glob.glob('tmpdeb/usr/lib/nremote/libdrmtap.so.0.*')
             + glob.glob('tmpdeb/usr/share/nremote/lib/libnremote.so')
             + glob.glob('tmpdeb/usr/share/nremote/nremote')
...
def retarget_control_to_drm_variant():
    ...
    out = []
    for line in lines:
        if line.startswith('Package: nremote'):
            out.append(f'Package: {DRM_PACKAGE_NAME}\n')
            out.append('Conflicts: nremote\nReplaces: nremote\nProvides: nremote\n')
            ...
            out.append(line)
    body = ''.join(out)
    # Fail loudly rather than silently shipping a package that says `nremote`: a stock control file
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/nremote')
    system2('mkdir -p tmpdeb/usr/share/nremote/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/nremote || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/nremote/')
    system2(
        'cp ../res/nremote.service tmpdeb/usr/share/nremote/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/nremote.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/nremote.svg')
    system2(
        'cp ../res/nremote.desktop tmpdeb/usr/share/applications/nremote.desktop')
    system2(
        'cp ../res/nremote-link.desktop tmpdeb/usr/share/applications/nremote-link.desktop')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/nremote/files/polkit && chmod a+x tmpdeb/usr/share/nremote/files/polkit")
    ...
    system2('dpkg-deb -b tmpdeb nremote.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('nremote.deb', '../nremote-%s.deb' % version)
    if ships_so:
        # Named apart from the stock package so installing the consent-free variant is a deliberate act.
        os.rename('../nremote-%s.deb' % version, f'../{DRM_PACKAGE_NAME}-{version}.deb')
    os.chdir("..")


DRMTAP_DLOPEN_MARKER = b'/usr/lib/nremote/libdrmtap.so.0'
...
def _carries_drmtap_marker(path, marker=DRMTAP_DLOPEN_MARKER):
    ...
    # libnremote.so is ~45 MB and there is no reason to hold it all in memory, and the `with`
...
def assert_staged_binary_is_drm():
    ...
    """
    binaries = [p for p in glob.glob('tmpdeb/usr/share/nremote/lib/libnremote.so')
                + glob.glob('tmpdeb/usr/share/nremote/nremote') if os.path.isfile(p)]
...
def build_deb_from_folder(version, binary_folder, want_drm=False):
    ...
    system2('rm tmpdeb/usr/bin/nremote || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/nremote/')
    system2(
        'cp ../res/nremote.service tmpdeb/usr/share/nremote/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/nremote.png')
    ...
    bundled_glob = glob.glob('tmpdeb/usr/share/nremote/libdrmtap.so.0.*')
    ...
    if want_drm:
        ...
        if bundle_carries_so:
            ...
            system2(f'rm -f "{so}"')
            system2('rm -f tmpdeb/usr/share/nremote/libdrmtap.so tmpdeb/usr/share/nremote/libdrmtap.so.0')
        else:
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibnremote.dylib target/release/libnremote.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/NRemote.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"NRemote Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon NRemote.app 200 190 --hide-extension NRemote.app nremote.dmg ./build/macos/Build/Products/Release/NRemote.app")
    os.rename("nremote.dmg", f"../nremote-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libnremote.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --locked --features {features} --lib --release')
        if not os.path.exists("target/release/libnremote.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/nremote.exe')
    os.chdir('../..')
    if os.path.exists('./nremote_portable.exe'):
        os.replace('./target/release/nremote-portable-packer.exe',
                   './nremote_portable.exe')
    else:
        os.rename('./target/release/nremote-portable-packer.exe',
                  './nremote_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/nremote_portable.exe')
    os.rename('./nremote_portable.exe', f'./nremote-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/nremote-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/nremote.exe')
        system2('mv target/release/nremote.exe target/release/NRemote.exe')
        pa = os.environ.get('P')
        if pa:
        ...
        os.makedirs(res_dir, exist_ok=True)
        system2(
            f'cp -rf target/release/NRemote.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/nremote-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/nremote-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --locked --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/nremote')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv nremote-%s-0-x86_64.pkg.tar.zst nremote-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./nremote.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/nremote-%s-0.x86_64.rpm ./nremote-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall nremote.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/nremote-%s-0.x86_64.rpm ./nremote-%s-suse.rpm' % (
                version, version))
        # yum localinstall nremote.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/nremote*.deb ./flutter/nremote.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo --locked bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/NRemote.app/Contents/MacOS/nremote')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/NRemote.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/nremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/NRemote.app/Contents/MacOS/nremote
    #rcodesign sign --p12-file ~/.p12/nremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/NRemote.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/nremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/NRemote.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/NRemote.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/NRemote.app
    '''.format(pa))
                system2(
                    'create-dmg "NRemote %s.dmg" "target/release/bundle/osx/NRemote.app"' % version)
                os.rename('NRemote %s.dmg' %
                          version, 'nremote-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/nremote-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./nremote-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./nremote-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple nremote-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/NRemote.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/nremote*.deb ./nremote.deb')
                system2('dpkg-deb -R nremote.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/nremote/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2('strip tmpdeb/usr/bin/nremote')
                system2('mkdir -p tmpdeb/usr/share/nremote')
                system2('mv tmpdeb/usr/bin/nremote tmpdeb/usr/share/nremote/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/nremote/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb nremote.deb; /bin/rm -rf tmpdeb/')
                os.rename('nremote.deb', 'nremote-%s.deb' % version)
