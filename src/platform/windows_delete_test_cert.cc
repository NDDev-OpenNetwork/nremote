...
//*************************************************************
//
//  DeleteNRemoteTestCertsW_SingleHive()
//
//  Purpose:    Deletes NRemote Test certificates and wrong key stores
//
...
BOOL DeleteNRemoteTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteNRemoteTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteNRemoteTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteNRemoteTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteNRemoteTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
