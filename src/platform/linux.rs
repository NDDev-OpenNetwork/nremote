...
lazy_static::lazy_static! {
    ...
    static ref SUDO_E_PRESERVES_ENV: bool = {
        if !is_root() {
            ...
            let key = format!("__NREMOTE_SUDO_E_TEST_{}", std::process::id());
            let val = "1";
...
/// An explicit `NREMOTE_FORCED_DISPLAY_SERVER` is an operator override, and the root service
...
pub(crate) fn display_server_forced() -> bool {
    std::env::var("NREMOTE_FORCED_DISPLAY_SERVER").is_ok()
}
    ...
    match desktop {
        Some(desktop) => {
            ...
            if let Ok(forced_display_server) =
                std::env::var("NREMOTE_FORCED_DISPLAY_SERVER")
            {
                if !forced_display_server.is_empty() {
                    envs.push((
                        "NREMOTE_FORCED_DISPLAY_SERVER",
...
// stop_nremote_servers() is just a temp solution here.
fn force_stop_server() {
    stop_nremote_servers();
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill nremote --tray, execute cp first
...
fn get_shortcuts_inhibitor_app_id() -> String {
    if is_flatpak() {
        ...
        // (e.g., "com.nremote.NRemote"). This is the most reliable source.
