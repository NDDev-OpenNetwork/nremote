...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in NRemote
            // 2. Move the physical mouse
            // 3. Press "V" key in NRemote
            ...
            // we consider only NRemote operates the controlled side.
            //
...
#[cfg(target_os = "macos")]
fn key_sleep() {
    // https://www.reddit.com/r/nremote/comments/1kn1w5x/typing_lags_when_connecting_to_macos_clients/
    //
    ...
    // Though `/Applications/NRemote.app/Contents/MacOS/nremote --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run nremote from cmd
        std::thread::spawn(|| {
