...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_nremote() {
            if s.contains("NRemote")
                && !name.starts_with("upgrade_nremote_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("NRemote") {
                    s = s.replace("NRemote", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "NRemote" (e.g., "NRemote-Admin"), we need to avoid
                    // replacing "NRemote" within the already-substituted app_name, which would
                    // cause duplication like "NRemote-Admin" -> "NRemote-Admin-Admin".
                    //
