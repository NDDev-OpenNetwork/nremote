...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Ready", "準備完了"),
        ("Established", "接続完了"),
        ("connecting_status", "NRemote ネットワークに接続中..."),
        ...
        ("install_tip", "UAC の影響により、NRemote がリモートデスクトップ上で正常に動作しない場合があります。UAC を回避するには、下のボタンをクリックしてシステムに NRemote をインストールしてください。"),
        ("Click to upgrade", "アップグレード"),
        ("Configure", "設定"),
        ("config_acc", "リモートからあなたのコンピューターを操作するには、NRemote に「アクセシビリティ」権限を与える必要があります。"),
        ("config_screen", "リモートからあなたのコンピューターにアクセスするには、NRemote に「画面録画」の権限を与える必要があります。"),
        ("Installing ...", "インストール中..."),
        ("Install", "インストール"),
        ("Installation", "インストール"),
        ...
        ("Discovered", "発見済み"),
        ("install_daemon_tip", "起動時に NRemote を開始するには、システムサービスをインストールする必要があります。"),
        ("Remote ID", "リモート ID"),
        ("Paste", "貼り付け"),
        ...
        ("android_input_permission_tip1", "この Android デバイスをリモートコンピューターからマウスやタッチで操作するには、NRemote に「ユーザー補助」からサービスの使用を許可する必要があります。"),
        ("android_input_permission_tip2", "次の端末設定ページに進み、「インストール済みアプリ」から「NRemote Input」を有効にしてください。"),
        ...
        ("Turned off", "オフになりました"),
        ("Language", "言語"),
        ("Keep NRemote background service", "NRemote バックグラウンドサービスを維持"),
        ("Ignore Battery Optimizations", "バッテリーの最適化を無効にする"),
        ("android_open_battery_optimizations_tip", "この機能を使わない場合は、NRemote アプリの設定ページから「バッテリー」に進み、「制限しない」を選択してください。"),
        ...
        ("elevated_foreground_window_tip", "リモートデスクトップでフォーカスされているウィンドウの操作にはより高い権限が必要なため、マウスとキーボードが一時的に使用できなくなっています。リモートユーザーにウィンドウを最小化、または接続管理画面から権限を昇格するよう要求してください。この問題を回避するには、リモートコンピューターに NRemote をインストールしてください。"),
        ("Disconnected", "切断しました"),
        ("Other", "その他"),
        ...
        ("software_render_tip", "Linux で NVIDIA 製のグラフィックカードを使用していると、接続後すぐにリモートウィンドウが閉じてしまう場合があります。オープンソースの Nouveau ドライバーに切り替えて、ソフトウェアレンダリングを使用するよう設定すると解決するかもしれません。(NRemote の再起動が必要です)"),
        ("Always use software rendering", "常にソフトウェアレンダリングを使用する"),
        ("config_input", "リモートコンピューターをキーボードで操作するには、NRemote に「入力監視」権限を与える必要があります。"),
        ("config_microphone", "リモートコンピューターと通話するには、NRemote に「音声録音」権限を与える必要があります。"),
        ...
        ("still_click_uac_tip", "リモートデスクトップユーザーが NRemote を実行する際に、UACを許可する必要があります。"),
        ...
        ("View Mode", "表示モード"),
        ("verify_nremote_password_tip", "NRemote のパスワードを確認する"),
        ...
        ("scam_text1", "もし、知らない相手から電話で NRemote のインストールやサービスの開始を依頼された場合、作業を進めずに、すぐに電話を切ってください。"),
        ...
        ("upgrade_nremote_server_pro_to_{}_tip", "NRemote Server Pro をバージョン {} 以上にアップグレードしてください！"),
        ...
        ("selinux_tip", "SELinuxが有効になっているため、NRemote が正常に動作しない可能性があります。"),
        ("Change view", "表示を変更"),
        ("Big tiles", "大きなタイル"),
        ("Small tiles", "小さなタイル"),
        ("List", "リスト"),
        ...
        ("preset_password_warning", "このエディションには、既定で固定パスワードが設定されています。このパスワードを知っているユーザーはあなたのデバイスを完全にコントロールできるため、そのような危険がある場合は直ちに NRemote をアンインストールして下さい！"),
        ...
        ("allow-only-conn-window-open-tip", "NRemote のウィンドウが開いている場合のみ接続を許可する"),
        ...
        ("floating_window_tip", "NRemote のバックグラウンドサービスを維持するために使用されます。"),
        ...
        ("upgrade_remote_nremote_client_to_{}_tip", "リモート側の NRemote クライアントをバージョン {} 以上にアップグレードしてください！"),
        ...
        ("allow-insecure-tls-fallback-tip", "既定では NRemote は TLS を使用するプロトコルのサーバー証明書を検証します。\nこのオプションを有効化すると NRemote は検証の手順をスキップして、検証に失敗した場合の処理を続行します。"),
        ("Disable UDP", "UDP を無効化する"),
        ("disable-udp-tip", "TCP のみ使用するかどうかを制御します。\nこのオプションを有効化すると、NRemote は UDP 21116 を使用せずに TCP 21116 を使用するようになります。"),
        ("server-oss-not-support-tip", "注意: NRemote Server OSS にはこの機能が含まれていません。"),
